const handleCollisions = (canvasObject, canvasX) => {
    ...
    collisionEnter.forEach((_collisionEnter) => {
        if (_collisionEnter.isCollision === false) {
            ...
            if (targetObject !== null) {
                ...
                const xCollision = Math.abs(object.position.x - target.position.x) <
                    (object.size.width + target.size.width) / 2;
                const yCollision = Math.abs(object.position.y - target.position.y) <
...
//# sourceMappingURL=handleCollisions.js.map